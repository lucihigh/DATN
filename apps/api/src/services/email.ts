...
export const sendCardDetailsOtpEmail = async (input: {
  to: string;
  recipientName: string;
  otpCode: string;
  expiresInMinutes: number;
  ...
  sendEmail({
    to: input.to,
    subject: "FPIPay card details verification code",
    text: `Your FPIPay card details OTP is ${input.otpCode}. It expires in ${input.expiresInMinutes} minutes.`,
    html: renderOtpEmailHtml({
      preheader: "Verify card details access with this one-time password.",
      title: "Card details verification code",
      subtitle:
        "We received a request to view sensitive card details in your FPIPay account.",
      recipientName: input.recipientName,
      otpCode: input.otpCode,
      expiresInMinutes: input.expiresInMinutes,
      summaryLabel: "Request type",
      summaryValue: "Secure card details access",
      securityNote:
        "If you did not request this access, do not share this code and review recent account activity immediately.",
    }),
    debug: { otpCode: input.otpCode },
  });
